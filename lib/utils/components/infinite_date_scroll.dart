...
class _InfiniteDateScrollState extends State<InfiniteDateScroll> {
  ...
  void initState() {
    ...
    DatesSession().isResetNotifier.addListener(_handleExternalDateChange);
  }

  @override
  void dispose() {
    _scrollController.dispose();
    DatesSession().isResetNotifier.removeListener(_handleExternalDateChange);
    super.dispose();
  }


  void _handleExternalDateChange() {
    final newDate = DatesSession().selectedDateNotifier.value;

    if (!_isSameDay(normalizeDate(DateTime.now()), newDate) && DatesSession().isReset) {
      setState(() {
        _selectedDate = DateTime.now();
      });


      DatesSession().setReset(false);
      DatesSession().setDate(DateTime.now());

      // Scroll to the new date
      final targetOffset = _indexFromDate(DateTime.now()) * itemExtent -
          (MediaQuery.of(context).size.width - itemExtent) / 2.1;
