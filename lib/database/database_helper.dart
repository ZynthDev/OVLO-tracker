...
import '../services/supabase/supabase_period_service.dart';
...
class DatabaseHelper {
  ...
  void bulkInfo() async {
    if(kDebugMode) {
      print(await _getTotalLoggedCycles());
      print((await _getMostFrequentSymptom())?.toSentenceCase());
      print((await _getMostFrequentMood())?.toSentenceCase());
      print(await _getMoodDistribution());
      print(await _getTop3Symptoms());
      print(await _getSymptomLogs());
      print(await _getMoodLogs());
      print(await _getAllCycles());
    }
  }
  ...
  Future<List<Map<String, String>>> _getMoodDistribution() async {
    ...
    final List<Map<String, String>> distributionList = [];
    ...
    moodCounts.forEach((mood, count) {
      ...
      distributionList.add({
        'mood': mood.toSentenceCase(),
        'emoji': categoryEmojis[mood.toSentenceCase()] ?? '',
  ...
  Future<List<Map<String, dynamic>>> _getTop3Symptoms() async {
    ...
    return top3.take(3).map((entry) {
      return {
        'symptom': entry.key.toSentenceCase(),
        'count': entry.value,
  ...
  Future<List<Map<String, dynamic>>> _getAllCycles() async {
    final db = await database;
    final List<Map<String, dynamic>> result = await db.rawQuery('''
      SELECT 
        ...
        JULIANDAY(MAX(date)) - JULIANDAY(MIN(date)) + 1 AS duration,
        GROUP_CONCAT(flow) AS flows
      FROM period_logs
      WHERE cycle_number IS NOT NULL
      GROUP BY cycle_number
      ORDER BY CAST(cycle_number AS INTEGER)
    ''');

    List<Map<String, dynamic>> cycles = result.map((row) => _parseCycleRow(row)).toList();

    return cycles;
  }


  Map<String, dynamic> _parseCycleRow(Map<String, dynamic> row) {
    final flows = row['flows']?.toString().split(',') ?? [];

    String flowIntensity = 'none';
    if (flows.contains('heavy')) {
      flowIntensity = 'heavy';
    } else if (flows.contains('medium')) {
      flowIntensity = 'medium';
    } else if (flows.contains('light')) {
      flowIntensity = 'light';
    } else if (flows.any((f) => f.trim().isNotEmpty)) {
    ...
    return {
      ...
      'duration': (row['duration'] as double).round(),
      'flow_intensity': flowIntensity.toSentenceCase(),
    };
  }
}
